...
import { type User, type TenVaiTro } from '@/interface/User'; // Import TenVaiTro
...
const AdminUserManagement = () => {
    // ⭐️ GỌI HOOK VÀ LẤY changeActive ⭐️
    ...
    // ⭐️ CẬP NHẬT: Xử lý chuyển đổi trạng thái (GỌI API) ⭐️
    const handleToggleStatus = async (email: string, currentStatus: boolean, vaiTro: TenVaiTro) => {
        // Nếu là Admin, không làm gì cả (UI đã vô hiệu hóa nút)
        if (vaiTro === 'Admin') {
    ...
    return (
        <div className="p-8 bg-gray-50 min-h-screen">
            {/* ... (Phần Header, Filter Bar) ... */}
            <h1 className="text-3xl font-semibold text-slate-900 mb-2">User Management</h1>
            <p className="text-sm text-slate-600 mb-8">
                Quản lý tất cả người dùng, kiểm soát quyền và trạng thái hoạt động.
            </p>
                    ...
                    <tbody className="divide-y divide-slate-200">
                        ...
                        {users.map((user) => (
                            <UserRow 
                                key={user.email} 
                                user={user} 
                                // Cập nhật tham số truyền vào UserRow
