...
const ProductCard = ({
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    // Extract volume number from string (e.g., "50ml" -> 50)
    const volumeNumber = volume ? parseInt(volume.replace("ml", "")) : 50;

    // Extract filename from imageUrl
    const imageFilename = imageUrl.includes('/')
        ? imageUrl.split('/').pop() || ''
        : imageUrl;
      ...
      <article className="group flex h-full flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-md">
        ...
        <span className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">
          ...
          <button
              ...
              className="mt-2 w-full rounded-full border border-slate-200 bg-white px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-slate-700 transition hover:border-red-500 hover:bg-red-500 hover:text-white"
          >
            Thêm vào giỏ
          </button>
        </div>
